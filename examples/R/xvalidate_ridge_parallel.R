...
#wait_times = rpois( length(unique(folds)), 5)
...
for ( fold in unique(folds) ) {
    #Sys.sleep(5) # just to illustrate a point
...
tm1 = proc.time()
leave_out_rmse_foreach = foreach(fold=unique(folds), .packages = 'glmnet') %dopar% {
  # Sys.sleep(5) # Just to illustrate a point
  Sys.sleep(wait_times[fold + 1]) # wait an "unknown" number of seconds"
  fit = glmnet(X_train[folds != fold, ], Y_train[folds != fold, ], 
               alpha = 1, lambda = lambda)    
...
do_fold = function(fold) {
  #Sys.sleep(5) # just to illusrate a point
  Sys.sleep(wait_times[fold + 1]) # wait an "unknown" number of seconds"
  fit = glmnet(X_train[folds != fold, ], Y_train[folds != fold, ], 
               alpha = 1, lambda = lambda)    
...
#sum(wait_times[ 1:10 %% 2 == 1])
#sum(wait_times[ 1:10 %% 2 == 0])
tm1 = proc.time()
leave_out_rmse_mcl = mclapply(unique(folds), do_fold, 
